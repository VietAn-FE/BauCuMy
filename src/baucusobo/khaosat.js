...
function KhaoSatBCSB(props) {
  ...
  const [srQuest, setsrQuest] = useState(0);
  ...
  const getKhaoSatBang = (addr) => {
    ...
    fetch(
      config.domainApi + "/api-uselection.htm?m=get_exit_polls",
      requestOptions
    )
      .then((response) => {
        return response.json();
    ...
    <div className="box-ks mb-30">
      <div className="box-vote-top">
        <div className="mflex-top">
          <h3 className="title">Khảo sát thăm dò ý kiến cử tri</h3>
        ...
        <p className="sapo">
          Khảo sát chỉ mang tính chất tham khảo trên một phần nhỏ cử chi tham
          gia bầu cử và kết quả trên không đại diện cho toàn bộ cử chi và người
          dân và Mỹ <br />
          Thống kê chỉ hiển thị top 2 ứng viên có nhiều cử tri bầu chọn nhất
          trong số người tham gia khảo sát và các nhóm đối tượng có tỷ lệ thấp
          hơn 0.1% sẽ hiển thị n/a
        </p>
            ...
            <div className="box-opinion" key={idx}>
              <div className="table-scroll">
                        ...
                        .map((itemCol, colidx) => {
                          return (
                            <div className="col-vlue" key={colidx}>
                              <span className="number">{itemCol.label}</span>
                ...
                <div className="box-row-author">
                  <div
                    className={`row-item ${
                      item?.table_answers.filter((n) => n[0] == "Đảng cộng hòa")
                        .length > 0
                        ? ""
                        : "hidden"
                    }`}
                  ...
                  <div
                    className={`row-item ${
                      item?.table_answers.filter((n) => n[0] == "Đảng dân chủ")
                        .length > 0
                        ? ""
                        : "hidden"
                    }`}
                  ...
                  <div className="row-update">
                    <span className="text-t">
                      {item.respondents.toLocaleString()} người tham gia khảo
                      sát
                    </span>
